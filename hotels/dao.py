...
class HotelsDAO(BaseDAO):
    ...
    async def get_all_rooms_with_staticstics(
        date_from: date, date_to: date, location: str
    ):
        async with async_session_maker() as session:
            left_rooms = (
                select(
                    Rooms.id.label("room_id"),
                    func.count(Bookings.room_id).label("cnt_bk"),
                )
                .select_from(Rooms)
                ...
                .where(
                    or_(
                        and_(
                            date_to > Bookings.date_to, date_from < Bookings.date_from
                        ),
            ...
            query = (
                select(
                    ...
                    func.coalesce(
                        Rooms.quantity - left_rooms.c.cnt_bk, Rooms.quantity
                    ).label("empty_rooms"),
                )
                .select_from(Rooms)
                .join(left_rooms, left_rooms.c.room_id == Rooms.id, isouter=True)
