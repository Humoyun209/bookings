...
async def test_register_user(
    email: EmailStr, password: str, status: int, ac: AsyncClient
):
...
async def test_login_user(email: EmailStr, password: str, status: int, ac: AsyncClient):
    response = await ac.post("/auth/login", json={"email": email, "password": password})
    assert response.status_code == status


@pytest.mark.parametrize(
    "email", ["humoyun209@gmail.com", "h.ahmedov209@gmail.com", "error@gmail.com"]
)
